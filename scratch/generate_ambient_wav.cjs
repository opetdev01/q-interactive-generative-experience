...
for (let i = 0; i < totalSamples; i++) {
  ...
  for (let n = 0; n < chord1.length; n++) {
    ...
    const oscSub = Math.sin(2 * Math.PI * (freq * 0.5) * t) * 0.35; // Deep sub-warmth
    
    // Slow LFO filter modulation
    const lfo = 0.6 + 0.4 * Math.sin(2 * Math.PI * 0.125 * t + n);
    
    sample += (osc1 + osc2 + oscSub) * (0.14 / chord1.length) * lfo;
  }

  // Synthesizer voice 2: High shimmer / aerial breeze (soft filtered sine octave shimmer)
